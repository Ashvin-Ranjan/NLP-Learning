...
pub struct ReverseSummationNode {
    ...
    pub children: HashMap<u16, ReverseSummationNode>,
}

impl ReverseSummationNode {
    // This adds a reverse n-gram to the tree structure
    pub fn add_reverse_ngram(&mut self, reverse_ngram: &[u16]) {
        self.count += 1;
    ...
    pub fn query_reverse_ngram(&self, reverse_ngram: &[u16]) -> Option<&ReverseSummationNode> {
        if reverse_ngram.len() == 0 {
            return Some(self);
        }
    ...
    pub fn serialize(&self) -> Vec<u8> {
        ...
        serial_data.append(&mut (self.children.keys().len() as u16).to_le_bytes().to_vec());
...
fn _deserialize(data: &[u8]) -> Result<(ReverseSummationNode, usize), u8> {
    ...
    let mut index_out: usize = 12; // This is because the first 12 bytes are standard
    let mut out = ReverseSummationNode {
        id: u16::from_le_bytes(data[0..2].try_into().expect("Slice with incorrect length")),
        count: u64::from_le_bytes(data[2..10].try_into().expect("Slice with incorrect length")),
        children: HashMap::new(),
    };
